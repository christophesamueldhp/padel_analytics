...
class BallTracker(Tracker):
    ...
    def __init__(
        ...
        tracknet_ckpt = torch.load(
            tracking_model_path, map_location=torch.device("cpu")
        )
        ...
        if inpainting_model_path:
            inpaintnet_ckpt = torch.load(
                inpainting_model_path, map_location=torch.device("cpu")
            )
    ...
    def draw_multiple_frames(
        self, frames: list[np.ndarray], ball_detections: list[Ball], traj_len=8
    ):
    ...
    def modify_pred_dict(self, pred_dict: dict):
        ...
        return {k: pred_dict[v] for k, v in mapping.items()}
    ...
    def predict_frames(
        ...
        for x in tqdm(data_loader):
            ...
            for sample_i in range(batch_size):
                if sample_count < buffer_size:
                    ...
                    y_pred = (
                        y_pred_buffer[sample_indices + sample_i, frame_indices]
                        * weight[:, None, None]
                    ).sum(0)
                ...
                if sample_count == num_sample:
                    ...
                    for frame_i in range(1, seq_len):
                        y_pred = y_pred_buffer[
                            sample_indices + sample_i + frame_i, frame_indices
        ...
        if self.inpaintnet is not None:
            ...
            # Define device dynamically based on CUDA availability
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

            for i, coor_pred, inpaint_mask in tqdm(data_loader):
                # coor_pred, inpaint_mask = coor_pred.float(), inpaint_mask.float()
                coor_pred, inpaint_mask = coor_pred.float().to(
                    device
                ...
                # Thresholding
                th_mask = (coor_inpaint[:, :, 0] < self.COOR_TH) & (
                ...
                # Thresholding
                th_mask = (ensemble_coor_inpaint[:, :, 0] < self.COOR_TH) & (
        ...
        pred_dict = (
            inpaint_pred_dict if self.inpaintnet is not None else tracknet_pred_dict
        )
    ...
    def predict_frames_single(self, frame_buffer):
        """
        Perform prediction on a buffer of frames of length seq_len, then produce a single prediction (for the last frame in the window).
        """
        # Convert frame_buffer to the format required by the model
        # Use BallTrajectoryIterable (as usual) to generate the batch
        iterable = BallTrajectoryIterable(
            seq_len=self.tracknet_seq_len,
            sliding_step=1,
            data_mode="heatmap",
            bg_mode=self.bg_mode,
            frame_generator=iter(frame_buffer),
            HEIGHT=self.HEIGHT,
            WIDTH=self.WIDTH,
            SIGMA=self.SIGMA,
            IMG_FORMAT=self.IMG_FORMAT,
        ...
        # Convert iterable to a single batch (only one window)
        x = next(iter(iterable))
        x = torch.tensor(x).unsqueeze(0).float().to(self.DEVICE)  # [1, C, H, W]
        with torch.no_grad():
            y_pred = self.tracknet(x).detach().cpu()

        # Take prediction for the last frame in the window
        indices = torch.tensor([[[0, i] for i in range(self.tracknet_seq_len)]])
        pred_dict = predict(
            indices,
            self.WIDTH,
            self.HEIGHT,
        ...
        # Take only the last prediction result
        x, y, vis = pred_dict["X"][-1], pred_dict["Y"][-1], pred_dict["Visibility"][-1]
        frame = len(pred_dict["Frame"]) - 1
        ball_obj = Ball(frame=frame, xy=(x, y), visibility=vis)
        return ball_obj
